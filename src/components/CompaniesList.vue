  ...
  <div class="list row">
    ...
    <div class="col-md-6">
      <div v-if="currentCompanie">
        <h4>Compañia</h4>
        <div>
          <br><label><strong>Nombre:</strong></label> {{ currentCompanie.name }}
        </div>
        <router-link :to="'/companies/' + currentCompanie.id" class="btn btn-info"> Editar</router-link>
      </div>
      <div v-else>
        <br />
        <p> Selecciona una Compañia.</p>
        <router-link to="/add-companie" class="btn btn-info">Agregar Compañia</router-link>
      </div>
    </div>
  </div>
</template>
